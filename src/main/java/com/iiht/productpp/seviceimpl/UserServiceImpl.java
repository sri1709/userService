...
public class UserServiceImpl implements UserServices{
	...
	public ResponseEntity<AuthResponse> login(LoginDetails loginDetails) {
		...
		if (user.isPresent() && userdetails.getPassword().equals(loginDetails.getPassword())) {
			...
			return new ResponseEntity<>(new AuthResponse(uid,true,generateToken,user.get().getFirstName(),user.get().getLastName(),user.get().getRole()), HttpStatus.OK);
		} else {
			log.info("At Login : ");
			log.error("Not Accesible");
			return new ResponseEntity<>(new AuthResponse(loginDetails.getUsername(),false,"In Correct Password",null,null,null), HttpStatus.FORBIDDEN);
		}
	}
